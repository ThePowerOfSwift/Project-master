//
//  HELanguageViewController.swift
//  Project
//
...
class HELanguageViewController: HEBaseViewController {

    var dataSource: [HELanguageModel]?
    let manager: HELanguageManager = HELanguageManager()
    ...
    override func viewDidLoad() {
        ...
        let currentLanguage: String = HELanguageHelper.getLanguage()
        for i in 0..<data.count {
            let one = data[i]
            let isSelected = currentLanguage == one.1
            let lang = HELanguageModel(title: one.0, language: one.1, isSelected: isSelected)
            manager.add(language: lang)
        }
        
        self.dataSource = manager.getLanguages()
        self.tableView.register(HEChooseLanguageCell.self, forCellReuseIdentifier: "cell")
    ...
    func showHUD(complete: @escaping (()->())) {
        let changeLanguageVC = HEChangeLanguageViewController()
        self.present(changeLanguageVC, animated: true, completion: complete)
    }
    
}

extension HELanguageViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! HEChooseLanguageCell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if !model.isSelected {
            ...
            self.showHUD(complete: {
                HELanguageHelper.setLanguage(language: model.language)
            })
        }
    }
}
