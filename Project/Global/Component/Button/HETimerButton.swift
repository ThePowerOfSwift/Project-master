...
//  Project
//
//  Created by weixhe on 2018/3/12.
...
@objc protocol HETimerButtonDelegate: class {
    @objc optional func buttonStateDidChanged(button: HETimerButton, noraml: Bool) -> Void
}

class HETimerButton: UIView {

    var max: Int = 10
    var delegate: HETimerButtonDelegate?
    var stateDidChanged: ((_ button: HETimerButton, _ noraml: Bool) -> ())?
    ...
    lazy private var timer: HETimer? = {
        let t = HETimer(delegate: self, begin: max, finish: 0)
        return t
    }()
...
extension HETimerButton : HETimerDelegate {
