...
protocol HETimerDelegate : class {
    func timer(timer: HETimer, counting: Int, finish: Int) -> Void
}


class HETimer: NSObject {

    weak var delegate: HETimerDelegate?
    ...
    // MARK: Init
    convenience init(timeInterval: TimeInterval = 1, delegate: HETimerDelegate?, increase: Bool = false, begin: Int, finish: Int, step: UInt = 1) {
        self.init()
    ...
    class func timer(timeInterval: TimeInterval = 1, delegete: HETimerDelegate?) -> HETimer {
        let t = HETimer(timeInterval: timeInterval, delegate: delegete, begin: 0, finish: 0)
        t.forever = true
        return t
    }
