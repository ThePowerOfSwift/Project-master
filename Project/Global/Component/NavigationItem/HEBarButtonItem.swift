...
open class HEBarButtonItem: NSObject {
    // MARK: Property
    var title: String? {
    ...
    // MARK: Init
    class func item(title: String, highlightedTitle: String?, target: Any?, action: Selector?) -> HEBarButtonItem {
        let item = HEBarButtonItem()
        item.title = title
    ...
    class func item(title: String, target: Any?, action: Selector?) -> HEBarButtonItem {
        return self.item(title: title, highlightedTitle: nil, target: target, action: action)
    }
    
    class func item(image: UIImage, highlightedImage: UIImage?, target: Any?, action: Selector?) -> HEBarButtonItem {
        let item = HEBarButtonItem()
        item.image = image
    ...
    class func item(image: UIImage, target: Any?, action: Selector?) -> HEBarButtonItem {
        return self.item(image: image, highlightedImage: nil, target: target, action: action)
    }
    
    class func item(title: String, image: UIImage, target: Any?, action: Selector?) -> HEBarButtonItem {
        let item = HEBarButtonItem()
        item.title = title
        item.image = image
        item.addAction(target, action)
