...
// MARK: - 1. 类：主标题+副标题
open class CVSubTitleView: UIView {
...
// MARK: - 2. 类：图标+标题
open class CVIconTitleView: UIView {
    ...
    private func updateFrame() -> Void {
        if let icon = self.icon {
            ...
            case .left:
                self.moveIcon(to: CVIconTitleView.ImageStyle.left)
            case .right:
                self.moveIcon(to: CVIconTitleView.ImageStyle.right)
            default:        // full
