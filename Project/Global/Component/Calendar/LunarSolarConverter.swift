...
class LunarSolarConverter {
     ...
     * |----4位闰月|-------------13位1为30天，0为29天|
     * */
    private class var lunar_month_days: [Int] {
        return [
    ...
    private class var solar_1_1: [Int] {
        return [
   ...
   private class func GetBitInt(_ data: Int, length: Int, shift: Int) -> Int {
        return (data & (((1 << length) - 1) << shift)) >> shift;
    }
    
    // WARNING: 日期1582年10月之前是不精确的
    private class func SolarToInt(_ y: Int, m: Int, d: Int) -> Int {
        let m = (m + 9) % 12;
        let y = y - m / 10;
        return 365 * y + y / 4 - y / 100 + y / 400 + (m * 306 + 5) / 10 + (d - 1);
    }
    
    private class func SolarFromInt(_ g: Int) -> Solar {
        ...
        let solar = Solar(year: y, month: mm, day: dd)
        return solar;
    }
    ...
    class func LunarToSolar(lunar: Lunar) -> Solar {
        ...
        let solar11 = LunarSolarConverter.solar_1_1[lunar.year - LunarSolarConverter.solar_1_1[0]];
    ...
    class func SolarToLunar(solar: Solar) -> Lunar {
        var lunar = Lunar();
        var index = solar.year - LunarSolarConverter.solar_1_1[0];
