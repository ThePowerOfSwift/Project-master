...
protocol HECycleScrollViewDelegate {
    func didSelectItem(atIndex: Int, model: HECycleScrollModel) -> Void
}

/// 闭包：点击了某一个item
typealias CycleCallBack = (Int, HECycleScrollModel) -> ()

class HECycleScrollView: UIView {
    ...
    var delegate: HECycleScrollViewDelegate?
    
    
    // MARK: 数据源
    var dataSource: Array<HECycleScrollModel>! {
        didSet {
    ...
    override init(frame: CGRect) {
        ...
        self.collectionView.register(HECycleCell.self, forCellWithReuseIdentifier: "Cell")
        ...
        self.dataSource = [HECycleScrollModel]()
    ...
    func defaultTimer() -> HETimer {
        let t = HETimer.timer(timeInterval: 1, delegete: self)
        t.start()
        counting = 0;
        return t
    }
...
extension HECycleScrollView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "Cell", for: indexPath) as! HECycleCell
...
extension HECycleScrollView: UIScrollViewDelegate {
    /// 在这个方法中算出当前页数
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        ...
        if autoScroll || infiniteLoop {
            HELog(message: "\(pageControl.currentPage)");
            self.collectionView.scrollToItem(at: IndexPath(item: pageControl.currentPage, section: 1), at: .left, animated: false)
        }
    }
}

extension HECycleScrollView: HETimerDelegate {
    func timer(timer: HETimer, counting: Int, finish: Int) {
        if autoScroll {
            self.nextImage()
        }
    }
